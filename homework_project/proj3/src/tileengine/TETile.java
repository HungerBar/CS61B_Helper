...
public class TETile {
    ...
    public static TETile[][] fromString(String input, Map<Character, TETile> tileMap) {
        // 分割字符串为行
        String[] lines = input.split("\n");
        lines = getLastNMinusOne(lines);
        if (lines.length == 0) {
        ...
        for (int y = 0; y < height; y++) {
            ...
            if (line.length() != width) {
                throw new IllegalArgumentException(
                        "Line " + y + " has invalid length: " + line.length() +
                                ", expected: " + width
                );
            }
            ...
            for (int x = 0; x < width; x++) {
                ...
                TETile tile = tileMap.get(c);

                if (tile == null) {
                    throw new IllegalArgumentException(
                            "Unmapped character '" + c + "' at position x=" + x + ", y=" + y
                    );
                }

                // 注意：Y坐标需要反转（原始toString是从上到下生成）
                world[x][height - 1 - y] = tile;
            }
        }

        return world;
    }
    private static String[] getLastNMinusOne(String[] array) {
